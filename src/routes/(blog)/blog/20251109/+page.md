...
앞서 `7장 영속성 어댑터 구현하기에 대한 보충`에서 논한 바와 같이, JPA로 헥사고날 아키텍처를 원칙적으로 구현하고자 할 경우 도메인 모델과 영속성 모델의 분리는 필수적이다. 이 경우 원저의 '양방향 매핑 전략'을 따르는 것이 좋다. 그러나 매핑 비용을 감수하기 어렵다면, 아웃고잉 포트의 계약은 포기하되 포트 자체를 생략하고 도메인 모델과 영속성 모델을 일치시키는 방안을 고려해볼 수 있다.
